...
gdp <- data.frame( "Country" = x, "GDP" = y)
gdp
```
...
ns<-select(newsuicide,country,sex,age,suicides_no,population)
n2s<-ns %>% group_by(country) %>% summarise(Total_Suicide=sum(suicides_no,na.rm = TRUE),Total_Population=sum(population,na.rm = TRUE)) %>% mutate(Suicide_Percentage=Total_Suicide/Total_Population)
